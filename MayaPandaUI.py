...
def MP_PandaVersion(option):
    ...
    """
    pm.melGlobals.initVar("string[]", "gMP_PandaFileVersions")
    executableToUse = ""
    selectedBamVersion = str(
        pm.optionMenu("MP_PY_BamVersionOptionMenu", query=1, value=1)
    )
    for i in range(0, len(pm.melGlobals["gMP_PandaFileVersions"])):
        if selectedBamVersion == pm.melGlobals["gMP_PandaFileVersions"][i]:
            if option == "getBam2Egg":
                executableToUse = pm.melGlobals["gMP_PandaFileVersions"][i + 1]
            # Get bam2egg file name

            elif option == "getEgg2Bam":
                executableToUse = pm.melGlobals["gMP_PandaFileVersions"][i + 2]
            # Get egg2bam file name

            elif option == "getPview":
                executableToUse = pm.melGlobals["gMP_PandaFileVersions"][i + 3]
            # Get pview file name

            i = len(pm.melGlobals["gMP_PandaFileVersions"])
    return executableToUse


def MP_ConfirmationDialog(title, message, type):
    """
...
def MP_AddEggObjectFlags(eggObjectType):
    """
    Add an egg-object-type to poly
    """
    pm.melGlobals.initVar("string[]", "gMP_EggObjectTypeArray")
    # global egg-object-type array
    # generate attribute enumeration list from the array
    enumerationList = ":".join(pm.melGlobals["gMP_EggObjectTypeArray"])
    ...
    eggTypeInArray = eggObjectType in pm.melGlobals["gMP_EggObjectTypeArray"]
    if eggTypeInArray == 1:
        indexNumber = -1
        ...
        for n in range(0, len(pm.melGlobals["gMP_EggObjectTypeArray"])):
            if pm.melGlobals["gMP_EggObjectTypeArray"][n] == eggObjectType:
                indexNumber = int(n)
            ...
            for node in selectedNodes:
                for i in range(1, 11):
                    if pm.objExists(str(node) + ".eggObjectTypes" + str(i)) == 1:
                        ...
                        pm.mel.MP_SetEggObjectTypeAttribute(
        ...
        pm.mel.MP_ConfirmationDialog(
            "Egg-Object-Type Error!",
            "The selected egg-object-type was not found in the $gMP_EggObjectTypeArray"
            + "\n"
            + "\nPlease verify the object-type being passed and update the "
            "$gMP_EggObjectTypeArray" + "\nto include the egg-object-type if the type "
            "is the correct one needed." + "\n" + "\nIf "
            "you "
            "modify "
            "the "
            "$gMP_EggObjectTypeArray DO NOT forget to update your PRC files."
...
def MP_TexPathOptionsUI():
    ...
    if selectedRB == "MP_ChooseDefaultTexPathRB":
    ...
    elif selectedRB == "MP_ChooseCustomRefPathRB":
        print("Custom Texture Reference Path Chosen\n")
        # Set options based on file type

        if outputPandaFileType == "MP_ChooseEggRB":
            pm.textField("MP_PY_CustomBamTexPathTF", edit=1, text="", enable=0)
        ...
        elif outputPandaFileType == "MP_ChooseEggBamRB":
    ...
    elif selectedRB == "MP_ChooseCustomTexPathRB":
...
def MP_OutputFilenameOptionsUI():
    """
    Updates the UI when a radio button is chosen
    """
    selectedRB = str(
    ...
    if selectedRB == "MP_ChooseOriginalFilenameRB":
    ...
    elif selectedRB == "MP_ChooseCustomFilenameRB":
...
def MP_TransformModeUI():
    ...
    if selectedRB == "MP_ChooseTransformNoneRB":
        print("Saves no transform information\n")

    elif selectedRB == "MP_ChooseTransformAllRB":
        print(
    ...
    elif selectedRB == "MP_ChooseTransformDCSRB":
        print(
            "Save transforms of objects which have dcs-flag only,"
            + "\nCLEARS local pivot, orientation, scale, "
            "and shear of the other objects." + "\nAll "
            "transform "
            ""
            "will be "
            "frozen "
    ...
    elif selectedRB == "MP_ChooseTransformModelRB":
        print(
...
def MP_SetEggObjectTypeAttribute(
    ...
    for i in range(1, 11):
        if pm.objExists(node + ".eggObjectTypes" + str(i)):
            if (
                pm.getAttr((node + ".eggObjectTypes" + str(i)), asString=1)
                == eggObjectType
            ):
                pm.mel.MP_ConfirmationDialog(
                    "Egg-Object-Type Error!",
...
def MP_AddEggObjectTypesGUI():
    ...
    count = 0
    for n in range(0, (len(pm.melGlobals["gMP_EggObjectTypeArray"]) + 1) / 4):
        pm.rowLayout(nc=6)
        for i in range(0, 5):
            eggObjectType = pm.melGlobals["gMP_EggObjectTypeArray"][count]
            # 5 rows per col
            if eggObjectType != "":
                annotation = str(pm.mel.MP_GetObjectTypeAnnotation(eggObjectType))
                # Get the defined annotation for egg-object-type
                pm.button(
                    width=100,
                    height=17,
                    command=lambda *args: pm.mel.MP_AddEggObjectFlags(
                        " + eggObjectType + "
                    ),
    ...
    pm.button(
        ...
        width=80,
        height=17,
        command=lambda *args: pm.mel.MP_UVScrolling("set"),
    ...
    pm.button(
        ...
        width=80,
        height=17,
        command=lambda *args: pm.mel.MP_UVScrolling("get"),
    ...
    pm.button(
        ...
        width=80,
        height=17,
        command=lambda *args: pm.mel.MP_UVScrolling("delete"),
...
def MP_GetEggObjectTypes():
    ...
    # Throw a message if at least one node has not been selected.
        ...
        pm.mel.MP_DeleteEggObjectTypesGUI(currentObjectTypesArray)




def MP_DeleteEggObjectTypesGUI(currentObjectTypesArray):
    pm.window(
    ...
    pm.button(
        ...
        width=110,
        height=20,
        command=lambda *args: pm.mel.MP_GetEggObjectTypes(),
        annotation=(
    ...
    # FIXME FIXME FIXME MISSING CONVERSION PART HERE
    for n in range(currentObjectTypesArray / 4):
        pass
...
def MP_ArgsBuilder(FileName):
    """
    constructs the arguments to pass to maya2egg
    """
    ARGS = "maya2egg"
    pm.melGlobals.initVar("string", "gMP_MayaVersionShort")
    ARGS += pm.melGlobals["gMP_MayaVersionShort"]
    ARGS += " "
    ...
    elif exportOptionsARGS == "MP_ChooseActorRB":
        ARGS += "-a model "

    elif exportOptionsARGS == "MP_ChooseAnimationRB":
        ARGS += "-a chan "
    ...
    elif exportOptionsARGS == "MP_ChooseBothRB":
        ARGS += "-a both "
        # set the start frames
        #  **Does not check if start frame < end frame
        #  **Does not support negative values
    ...
    elif exportOptionsARGS == "MP_ChoosePoseRB":
        ARGS += "-a pose "
    ...
    if transformMode == "MP_ChooseTransformModelRB":
        ARGS += "-trans model "

    elif transformMode == "MP_ChooseTransformAllRB":
        ARGS += "-trans all "

    elif transformMode == "MP_ChooseTransformDCSRB":
        ARGS += "-trans dcs "

    elif transformMode == "MP_ChooseTransformNoneRB":
        ARGS += "-trans none "
    ...
    if pm.textField("MP_PY_ForceJointTF", query=1, text=1) != "":
        ...
        for i in range(0, numberEntries):
            ...
            # If there is no DCS attribute, add it to the node
                ...
                pm.mel.MP_AddEggObjectFlags("dcs")
                mustRestart += 1
        ...
        if mustRestart != 0:
            ...
            if confirmRestart == "YES":
                pm.mel.MP_StartSceneExport()
                return "failed"
            # needed to exit the previous exporting loop

            else:
                pm.pm.mel.error("User cancelled exporting")
                return "failed"

    if (
        pm.radioCollection("MP_PY_TexPathOptionsRC", query=1, select=1)
...
def MP_StartSceneExport():
    ...
    if pm.checkBox("MP_PY_ExportSelectedCB", query=1, value=1):
        tempMBFile = str(pm.mel.MP_ExportScene("selected"))
        # returns $tempScenePath as $tempMBFile
        if tempMBFile == "failed":
            return 0

    else:
        tempMBFile = str(pm.mel.MP_ExportScene("all"))
        # returns $tempScenePath as $tempMBFile
        if tempMBFile == "failed":
            return 0

    origFileName = str(pm.mel.basenameEx(pm.pm.cmds.file(q=1, sceneName=1)))
...
def MP_ExportScene(selection):
    """
    exports the entire scene/selected objects
    """
    scenePath = str(pm.mel.dirname(pm.pm.cmds.file(q=1, sceneName=1)))
    # gets the current scene filename and path if present.
    fileName = str(pm.mel.basenameEx(pm.pm.cmds.file(q=1, sceneName=1)))
    # cut off the file extension
    fileExtension = str(pm.mel.fileExtension(pm.pm.cmds.file(q=1, sceneName=1)))
    ...
    if selection == "all":
        ...
        pm.pm.cmds.file(tempScenePath, ea=1, typ="mayaBinary", op="v=1")
        ...
        pm.pm.cmds.file(tempScenePath, typ="mayaBinary", es=1, op="v=1")
...
def MP_Globals():
    ...
    pm.melGlobals.initVar("string", "gMP_MayaVersionShort")
    pm.melGlobals["gMP_MayaVersionShort"] = str(
        pm.mel.substituteAllString(mayaVersionLong, ".", "")
    )
    # Strips the version zeroes if the version number length is less than 4
    if len(pm.melGlobals["gMP_MayaVersionShort"]) < 4:
        pm.melGlobals["gMP_MayaVersionShort"] = str(
            pm.mel.substituteAllString(pm.melGlobals["gMP_MayaVersionShort"], "0", "")
        )
    ...
    pm.melGlobals["gMP_PandaFileVersions"] = ["Default", "bam2egg", "egg2bam", "pview"]
    ...
    pm.melGlobals.initVar("string[]", "gMP_EggObjectTypeArray")
    pm.melGlobals["gMP_EggObjectTypeArray"] = []
    pm.melGlobals["gMP_EggObjectTypeArray"] = [
        "barrier",
        "barrier-no-mask",
        "ground",
...
def MP_CreatePandaExporterWindow():
    """
    ...
    # Process Variables
    pm.melGlobals.initVar("string", "gMP_MayaVersionShort")
    pm.melGlobals.initVar("string", "gMP_ReleaseRevision")
    # Exporter GUI Creation
    pm.window(
        "MP_PY_PandaExporter",
        sizeable=1,
        width=400,
        title=("Panda Exporter " + pm.melGlobals["gMP_ReleaseRevision"]),
        height=400,
        visible=0,
        retain=1,
    )
    ...
    pm.radioButton(
        "MP_PY_ChooseActorRB",
        onCommand=lambda *args: pm.mel.MP_ExportOptionsUI(),
    ...
    pm.radioButton(
        ...
        onCommand=lambda *args: pm.mel.MP_TransformModeUI(),
    ...
    pm.optionMenu(
        ...
        annotation=(
            ...
            "$gMP_PandaFileVersions Array as "
            "needed"
        ),
    )
    ...
    for i in range(0, len(pm.melGlobals["gMP_PandaFileVersions"])):
        pm.menuItem(label=pm.melGlobals["gMP_PandaFileVersions"][i])
        # Generate menu item
    ...
    pm.button(
        ...
        width=90,
        height=20,
        command=lambda *args: pm.mel.MP_AddEggObjectTypesGUI(),
        annotation=(
    ...
    pm.radioButton(
        ...
        onCommand=lambda *args: pm.mel.MP_TexPathOptionsUI(),
    ...
    pm.radioButton(
        ...
        onCommand=lambda *args: pm.mel.MP_OutputPathOptionsUI(),
    ...
    pm.radioButton(
        ...
        onCommand=lambda *args: pm.mel.MP_OutputFilenameOptionsUI(),
    ...
    pm.text(
        ...
        annotation=(
            ...
            + "\nAny spaces will be replaced with an "
            "underscore!"
        ),
    ...
    pm.textField(
        ...
        annotation=(
            "Character name associated with Model/Animation set"
            + "\nAny spaces will be replaced with "
            "an underscore!"
        ),
        width=170,
    )
    ...
    pm.radioButton(
        ...
        onCommand=lambda *args: pm.mel.MP_AnimationOptionsUI("animationMode", ""),
        enable=0,
    ...
    pm.intField(
        ...
        width=40,
        step=1,
        changeCommand=lambda *args: pm.mel.MP_AnimationOptionsUI(
    ...
    pm.intScrollBar(
        ...
        width=33,
        step=1,
        dragCommand=lambda *args: pm.mel.MP_AnimationOptionsUI(
    ...
    pm.button(
        ...
        width=110,
        height=20,
        command=lambda *args: pm.mel.MP_StartSceneExport(),
        annotation=(
...
pm.menuItem(
    command=lambda *args: pm.mel.MP_PandaExporterUI(), label="Panda Export GUI..."
)
pm.menuItem(
    command=lambda *args: pm.mel.MP_GetFile2Pview(), label="View file in PView..."
)
pm.menuItem(
    command=lambda *args: pm.mel.MP_AddEggObjectTypesGUI(),
    label="Add Egg-Type Attribute",
)
pm.menuItem(command=lambda *args: pm.mel.MP_GotoPanda3D(), label="Panda3D Home")
pm.menuItem(command=lambda *args: pm.mel.MP_GotoPanda3DManual(), label="Panda3D Manual")
pm.menuItem(
    command=lambda *args: pm.mel.MP_GotoPanda3DForum(), label="Panda3D Help Forums"
)
pm.menuItem(
    command=lambda *args: pm.mel.MP_GotoPanda3DSDKDownload(),
...
pm.melGlobals.initVar("string", "gMP_ReleaseRevision")
pm.melGlobals["gMP_ReleaseRevision"] = "v1.9"
